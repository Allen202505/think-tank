...
- 最终 diagnosis.rows 选 6-10 项；P0 只留给首次出现、显著恶化或影响资金安全的变化，持续无变化的旧问题降为背景风险；每项必须有证据来源。`
...
- 先判断边际变化，再排优先级：首次出现或显著恶化的变化优先；明显改善的变化也要重点核查其来源和持续性；连续存在且本期没有新变化的旧问题降为背景风险，不能机械保持 P0。
- 对持续存在的问题，问题句要问“变化/持续性”，不能重复追问既定事实。例如经营现金流已连续为负但本期大幅改善，应问“经营现金流为什么大幅改善，改善能否持续？”，而不是“利润为什么没有变成现金？”。
- 顶部自动提炼 3 条结论：coreContradiction（核心矛盾）、mainRisk（主要风险）、keyLead（值得继续追踪的积极/中性线索）。三条都必须写成完整可读的中文句子，讲清楚“是什么风险/线索、为什么值得关注”，不能只罗列数字；数字只能作为补充证据。
- focus 只放 3-6 个 2-6 字的短标签（如“利润质量”“现金流”“订单增长”），不要放完整结论句。
- rows 数量 6-10 条，按 P0、P1、P2 排序；P0 只留给首次出现、显著恶化或直接影响资金安全的变化，数量可少至 0 条、最多 3 条；P1 2-5 条，P2 1-3 条。优先级只代表调查顺序，不代表股票评级。
...
{"content":"你的解读发言（分段、带加粗）","followUps":["完整的问句1？","完整的问句2？"],"diagnosis":{"company":"公司名称（代码）","businessModel":"一句话经营模式","focus":["利润质量","现金流","订单增长"],"coreContradiction":"利润与现金兑现长期存在时间差，但本期经营现金流明显改善，旧的现金质量矛盾正在发生变化。","mainRisk":"经营现金流仍为负，若改善主要来自季节性回款而非持续回款，资金周转压力可能再次放大。","keyLead":"经营现金流同比明显改善，值得继续验证改善来源和持续性。","rows":[{"priority":"P1","domain":"利润质量","question":"经营现金流为什么大幅改善，改善能否持续？","evidence":["经营现金流 -6.86 亿元","去年同期 -29.12 亿元，同比减亏约 76.44%","归母净利润 3.98 亿元"],"judgment":"现金流出现明显边际改善，但绝对值仍为负，旧问题仍在；当前更应核查改善来源和持续性，暂不能直接定性为回款改善。","nextCheck":{"what":"查现金流量表附注、应收、合同资产和存货","lookAt":"看销售商品收现、项目结算和季节性回款的变化","judge":"判断经营现金流改善是持续性回款改善，还是一次性或季节性因素"},"status":"watch","source":"三表 / 年报附注"}],"topQuestions":["经营现金流为什么大幅改善，改善能否持续？","问题2？","问题3？"],"coverage":{"structured":0,"filing":0,"missing":0},"asOf":"2025年报"}}
