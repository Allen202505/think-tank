...
export function buildFrameworkStepPrompt(news, host, guests, step, prevSteps) {
  ...
  let jsonShape;
  if (step.key === 'gate') {
    speechFocus = '这是初筛块，要快：用 2-4 句给出相关性判断，发言里明确亮出 🟢/🟡/⚪ 标记和一句话理由。';
    ...
    speechFocus = '这是全桌收束块：用一段自然发言收尾，必须把「最反直觉的一句话结论、具体受益标的（名称+代码）、可不可以买/买入区间/止损位/持仓周期、风险提醒」都说进发言里，不要用列表。';
...
export function buildQuickTurnPrompt(news, host, guests, turnKey, prevTurns) {
  ...
  if (turnKey === 'host_open') {
    ...
    role = host;
    instruction = '收束：给出 🟢/🟡/⚪ 相关性标记 + 是否值得深挖的一句话理由（≤100字），要回应前面嘉宾的观点。';
    jsonShape = '{"text":"你的收束（≤100字）"}';
  } else if (turnKey === 'summary') {
    role = host;
    instruction = '总结：2-4 句话、150-250 字，**不受 100 字限制**。说清这条新闻的相关性结论、值不值得深挖、如果深挖重点看什么，并把前面各位的观点收进来。';
    jsonShape = '{"summary":"巴菲特最后总结（150-250字）"}';
  } else {
    const m = /^guest(\d+)$/.exec(turnKey || '');
    const idx = m ? Number(m[1]) : 0;
    role = (guests[idx] && guests[idx].master) || host;
    instruction = '一句快速观点（≤100字）：从你的流派视角给判断，可以补充或反驳前面的人，像早餐桌上抢话。';
