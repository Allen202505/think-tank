...
> 最后更新：2026-09-11（新增大师实盘联赛，并将四个专项模块合并为功能箱）
...
- 模糊搜索使用 `GET /api/stock-search?q=关键词`，返回 A 股名称、代码与 secid；用于行业周期输入框候选下拉。
...
- 当前内测赛季交易计划为 `src/data/masterLeague.js` 中的预置序列，但执行价、持仓市值、净值曲线和排名全部由公开真实日线计算。
- 每个决策显示理由、风险、是否改变此前判断、执行状态和大师点评；点评包含点评人、点评对象与本人的回怼，语气犀利、有梗但不对人；每位大师至少被 3 位不同大师点评，避免评论区只有两种声音。
- 互评由 AI 现场生成（`masterLeagueCommentary.mjs`）：输入是对方今天的真实操作、最新持仓、排名与当天大盘，输出「点评 + 本人回怼」。约束：只用自己的方法论说话、必须咬住具体操作与数字、**只能引用给定数字不许编造**、只怼方法不人身攻击。结果按天缓存，页面只读缓存（访客刷新不花钱），生成由定时任务或站长手动触发。
- 互评结果持久化到 `master_league_commentary` 底表（匿名可读、service_role 写入），读取优先级为「数据库 → 进程内存 → 预置文案」；重启/多实例不丢。
- 每个交易日 15:35（北京时间）由 Vercel Cron 自动生成六位大师的互评；周末与节假日跳过（周一~周五触发 + 交易日校验）。
...
- 大师智能体（规划）：每位大师 = 人设 + 记忆 + 工具 + 结构化决策。LLM 只输出 `{action, symbol, targetPct, reason, risk}`，成交价/持仓/收益仍由结算引擎按真实行情计算。
...
- 待办：6 位大师批量生成 + 幂等落 `master_league_decisions` + 收盘定时任务；把进程内成本台账升级为底表持久化。
- 线上库已执行 `supabase/master_league.sql`（2026-09-13）。
- 定时任务、幂等任务编排、公共点赞总数及历史赛季归档仍属于下一阶段。
