...
| 功能箱 | 左侧单一入口收纳鱼大基础面研究、芒格财报、缠中说禅、纳瓦尔知识学堂、选股策略大赏；鱼大位于第一个 Tab 并作为默认入口，箱内统一左对齐，不显示重复的“功能箱”标题和说明，兼容旧模块 URL |
...
- **微信联系二维码**：侧栏底部「入群聊一聊」是 Web 端唯一微信联系入口，打开个人微信二维码弹层；默认读取 `public/my-qr.jpg`，可通过 `NEXT_PUBLIC_QR_CODE_URL` 覆盖；原桌面侧栏和移动端顶部的独立微信图标不再展示
...
- 首批参赛者：杰西·利弗莫尔、理查德·D·威科夫、尼古拉斯·达瓦斯、杰拉尔德·勒布、安德烈·科斯托拉尼、伯纳德·巴鲁克。
...
- 交易日统计从 2026-09-14 首次计入，首日为第 1 个交易日；行情历史中开赛日之前的数据只能用于计算，不得进入比赛曲线、交易日计数或收益曲线起点。截至 2026-09-21，公开赛应为第 6 个交易日。
...
2. 买入按 100 股整数倍计算；生成计划时先按可用现金、目标价一手成本和同日卖出回笼资金校正。现金不足以覆盖一手时取消买入或降低目标仓位；执行引擎仍保留资金兜底，持仓不足时不产生卖出。
...
- 每日决策由 AI 自主生成（`masterLeagueDecisionJob.js`）：收盘后每位大师各跑一次智能体循环（看大盘、看板块、看自己的现金/持仓与历史操作），输出 1~3 个动作（买入/加仓/减仓/卖出/持有），落 `master_league_plans` 底表，次日开盘由结算引擎执行。买入/加仓必须覆盖一手成本；需要腾挪资金时先输出卖出/减仓，再输出买入/加仓。生成结果会按账户现金做二次约束，执行引擎同日固定先处理卖出/减仓。计划里的标的可以是全市场任意 A 股，接口会按计划动态拉取对应行情；拿不到行情的标的标记为未执行，不编造成交。
...
## 17. 股票池、大师联赛与基础面研究优化（2026-09-21）

### 17.1 股票池

- 「我的股票池」支持移除单只股票，不删除整个池子；桌面表格、移动端列表与移动端详情均可触发，确认后本地和云端池同步更新，并清除该股持仓成本。
- 宽表在桌面端只保留表格自身的横向滚动条，不再在表格上方重复显示滚动条。
- 用户可选择冻结前 `0-5` 列，默认 2 列；粘性列使用固定列宽计算偏移，选择结果本机持久化。表格使用 `border-collapse: separate` 确保 Chrome/Windows 下 `position: sticky` 真正生效。
- 桌面宽表采用自适应列宽：每列取「表头文字宽度（含排序箭头）」与「当前池实际内容最宽值」的最大值，再增加单元格内边距和额外冗余；表头额外预留 6px，避免 Chrome/Windows 因字体取整出现 `scrollWidth` 比 `clientWidth` 大 1px 的省略号。
- 股票池成功提示为轻提示，出现后 4 秒自动消失，不长期占用页面空间。
- 行情统计与持仓点评的加载态使用单颗 3D 骰子，骰子位于加载文案上方并垂直居中；`prefers-reduced-motion` 下关闭骰子动画。

### 17.2 大师实盘联赛

- 大师详情将当前持仓与历史持仓合并为一张「持仓记录」，新增「持仓状态」列，枚举值固定为「持仓中 / 已清仓」；下一交易日计划变动独立展示，投资策略历史使用另一个 Tab。收益曲线独占第一行，整个持仓区（含两个 Tab）移到曲线下方的整行区域，不再作为右侧窄栏展示。
- 大师详情的「持仓与变动 / 投资策略历史」使用与功能箱一致的胶囊页签：圆形图标、圆角边框、选中态金色描边浅底。
- 大师策略区和大师互评区中的大师头像均可点击进入对应大师详情；顶部大师头像点击进入详情，头像以外的名字/名次区域仍用于切换策略。
- 大师详情在大师姓名旁展示出生死亡年份与国籍标签；理查德·D·威科夫、安德烈·科斯托拉尼、伯纳德·巴鲁克使用全名展示，并补齐三位大师头像。伯纳德·巴鲁克简介扩写为生平、投资方法、公共经历和风险观念。
- 大师详情移除重复的「参赛者档案 · 年代 · 国籍」行；六位大师均提供两段式详细资料，介绍生平、市场经历、代表方法和风险观念。详细资料使用独立 `biography` 字段，智能体决策继续使用较短 `intro`，避免每次决策加载长篇传记。
- 大师名次胶囊只显示「第 N 名」文字，不再在文字前重复数字；大师头部上下两条分隔线移除，头像、姓名和简介区域直接留白衔接。
- 大师详情的收益曲线与收益数据合并为同一个左右分栏：左侧曲线，右侧以 2×2 展示总资产、累计收益、累计收益率和可用现金；桌面曲线固定约 190px 高，移动端自动上下排列。
- 投资策略历史默认每页 10 条，支持上一页、页码和下一页；当前未超过 10 条时只显示总数与当前页码信息。切换大师或切换详情 Tab 时自动回到第 1 页。
- 公开赛顶部刷新按钮保持使用标准 `RefreshCw` 刷新图标，骰子只保留在数据加载状态和读取数据胶囊中。
- 待执行买入明确显示为「尚未计入当前持仓」，避免把下一交易日计划误解为已成交持仓。
- 投资策略历史对每笔计划显式标注「待执行 / 已执行 / 未执行」，并展示执行日期；未执行原因固定可见。未执行计划不会进入「持仓记录」。
- 对目标股数大于 0、但现金不足一手的买入，结算状态改为 `skipped`，不再生成“0 股已执行”记录。
- 结算引擎从成交记录生成 `positionHistory`。股票清仓后记录不删除，保留清仓时间、清仓数量、清仓价格、清仓金额、已实现与累计收益。

### 17.3 鱼大基础面研究

- 左侧新增历史分析栏：使用股票筛选下拉框切换股票，下方为单层查询记录列表；每完成一次查询只生成并展示一条记录，点击记录恢复对应研究正文、评分和补充说明。
- 同一股票可以多次查询并保留多条不同时间的历史记录；不得再额外渲染股票分组卡片，避免一次查询看起来出现两条记录。
- 基础面研究加载时，使用与大师实盘联赛相同的 3D 摇骰子动效；骰子、五位大师头像、当前步骤文案和等待提示统一在内容区水平/垂直居中。
- 历史记录只保存在本机，最多保留最近 30 条；最近一次研究状态继续兼容原持久化键。

### 17.4 本地查询缓存

- 行情上下文、股票联想搜索和行业周期分析采用交易日感知缓存：盘中 5 分钟，盘前缓存到 09:15，收盘后与周末缓存到下一个工作日 09:15。
- 大师联赛公开赛数据优先使用本机缓存，最长 30 分钟；点击刷新会绕过缓存。
- 财报链接解读按“链接 + 补充说明”缓存 7 天；基础面研究同一股票 7 天内有历史结果时，直接恢复原记录，不重复调用 AI。
- 缠论分析按问题缓存到当前行情窗口结束；早餐新闻、纳瓦尔期数、选股池评级/区间继续使用已有专用缓存。
- 账户、免费额度、邀请/删除、用户池写入、实时快讯和雷达源不做长期本地缓存，避免权限、隐私与时效问题。

## 内容运营与站外引流（2026-09-23）

### 目标

- 通过知乎中短篇投资认知内容建立可信的站外获客入口。
- 让读者带着一个具体投资问题进入网站，并完成一次对应功能体验。
- 不用荐股、收益截图或短期行情预测换取流量。

### 运营约束

- 每篇内容只设置一个主承接模块，并记录对应 topic_id。
- 外部链接使用 UTM；首月以内容和工具体验数据为主，不预设阅读量或转化率硬指标。
- 涉及真实数据时必须带来源和日期；无法核验时只使用通用案例。
- AI 大师观点必须说明是公开方法模拟，不得伪造成本人发言。
- 文章不得包含目标价、收益承诺、确定性买卖指令或个股推荐。
- 发布、复用和复盘流程以 `memory/zhihu-content.md` 为准。


## 18. 选股策略大赏（2026-09-26）

### 18.1 产品范围

- 将用户提供的知乎高赞选股策略汇编转为站内内容模块，首版静态收录 18 条有可执行方法论的方法型策略。
- 策略分为长线价值、短线波段、交易心法三类；“名单股票池”栏目整块下线，神回复和纯观点回答也不纳入正式卡片。
- 每张卡片必须展示策略名称、类型、核心描述、可执行要点、答主/出处、原始链接及风险或时效提示。
- 预置策略详情必须从用户源汇编补充完整拆解，覆盖核心逻辑、执行步骤、买入/卖出条件、止损、仓位、案例、适用场景和时效限制；首版共 119 条细分信息。
- 支持按关键词、类型、热度、最近添加和“我的添加”筛选。
- 所有策略卡片使用统一宽度和一致的信息密度，不把首张或第二张卡片做成大卡、窄卡等特殊布局。

### 18.2 链接提取与保存

- 选股策略大赏作为功能箱第五个 Tab，通过 `?tab=toolbox&tool=strategy-gallery` 进入；旧 `?tab=strategy-gallery` 自动映射到该 Tab。
- 用户可粘贴公开 http(s) 链接，由服务端抓取正文并交给 AI 提取结构化策略。
- 链接要求公开可访问，内网、本机、超时、过大页面和非网页内容会被拒绝；链接抓取失败时允许粘贴原文。
- AI 输出必须经过预览和可编辑核对后才能加入“我的策略”，避免不可见地写入错误数据。
- 用户策略仅保存在当前浏览器，不发布到公共榜单；AI 仍复用现有 BYOK、免费额度和限流规则。

### 18.3 合规与边界

- 页面不把原作者观点包装为平台建议，不展示收益承诺，所有原文风险提示应保留。
- 赞数和问题浏览量为 2026-09-26 快照，页面必须说明数值会变化且内容不构成投资建议。
- 首版不做公共投稿、审核后台、跨设备同步和自动追踪原回答更新；后续如需扩展应另建内容审核与持久化方案。

## 19. 分析结果公开分享链接（2026-09-26）

### 19.1 产品目标

- 用户完成大师PK、巴菲特早餐或芒格财报分析后，可生成链接分享给他人。
- 接收方打开链接无需登录，直接阅读分享时的完整分析结果，而不是只能看到网站入口或推荐语。
- 分享内容保持“快照”语义：原结果后续重算、删除或本地缓存变化，不改变已生成链接中的内容。

### 19.2 覆盖范围

- 大师PK：问题、主持人开场/收束、大师发言、立场、核心观点、综合裁决和后续追问轮次。
- 巴菲特的早餐：原始新闻、快速/深度模式、嘉宾阵容、快速逐轮发言、深度框架步骤、机会池、核心结论和已完成追问。
- 芒格教你读财报：财报解读正文、继续追问建议、系统数据卡和一页纸财务诊断清单。
- 分享入口位于对应结果完成后的操作区；链接生成后自动复制，分享页提供再次分享和复制链接按钮。

### 19.3 安全与隐私

- 分享前递归移除 API Key、AI 配置、Base URL、上传文件内容和通用 token/password/secret 字段。
- 分享 ID 使用随机不可枚举字符串；公开链接无需登录，但数据表不向 anon/authenticated 开放直读，由服务端按 ID 查询。
- 分享页明确标注“分享时快照”和 AI 投资内容免责声明，搜索索引设为 `noindex`。
- 分享接口限制单条内容大小并按 IP 限流；数据库尚未初始化或 service role 未配置时明确失败，不静默生成仅本机可用的假链接。

### 19.4 边界

- 首版支持大师PK、巴菲特早餐和芒格财报三类，其他模块通过新增类型和查看器扩展。
- 首版不做账号管理、撤回、过期、访问密码、浏览统计和评论互动。
- 分享内容只用于公开只读阅读，不携带原用户登录态、用户股票池或其他本地数据。
